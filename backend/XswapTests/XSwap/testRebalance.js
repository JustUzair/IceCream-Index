...
async function main() {
  ...
  const _sender = await ethers.getSigner(
    "0x491E59c255C790D4e3a53CEC2632524088f1aaA4"
  );
  const _otherSender = await ethers.getSigner(
    "0x1383bc655CC8cC79c72cd0106023013c34Fd8086"
  );

  const aaveWhale = await ethers.getSigner(
    "0xb2c05c6e2ba1c68b24d5e840a5407b9f3bd9856c"
  );

  const wethWhale = await ethers.getSigner(
    "0x61d3bafb9bc3da723e4259a7e3653099aeb4487f"
  );
  ...
  await AAVE.connect(aaveWhale).approve(
    _sender.address,
    "150558000000000000000"
  );
  await WETH.connect(wethWhale).approve(
    _sender.address,
    "127530887721044054389"
  );
  ...
  const AAVEBalanceOwner = await AAVE.connect(aaveWhale).balanceOf(
    _sender.address
  );
  const wethBalanceOwner = await WETH.connect(wethWhale).balanceOf(
    _sender.address
  );
  ...
  const Pumpkin = await _Pumpkin.deploy(
    _sender.address,
  ...
  const AAVEPumpkinBalance = await AAVE.connect(aaveWhale).balanceOf(
    Pumpkin.address
  );
  const wethPumpkinBalance = await WETH.connect(wethWhale).balanceOf(
    Pumpkin.address
  );
  ...
  await Pumpkin.connect(_sender).rebalance(0, 1, "1000000");
  console.log("Swapping underlying tokens on XSwap...");
  console.log("done...");

  //const getPoolAddress = await WETH.connect(_sender).allowance(Pumpkin.address,"0xf0702249F4D3A25cD3DED7859a165693685Ab577");
  //console.log("Approval for pool: ", getPoolAddress.toString());

  //console.log("Pool address: ",getPoolAddress.toString());

  const AAVEPumpkinBalance2 = await AAVE.connect(aaveWhale).balanceOf(
    Pumpkin.address
  );
  const wethPumpkinBalance2 = await WETH.connect(wethWhale).balanceOf(
    Pumpkin.address
  );

  console.log(
    "Pumpkin AAVE Balance after Rebalance: ",
    AAVEPumpkinBalance2.toString()
  );
  console.log(
    "Pumpkin WETH Balance after Rebalance: ",
    wethPumpkinBalance2.toString()
  );

  const percentage1 = await Pumpkin.connect(_sender).getPercentages(0);
  const percentage2 = await Pumpkin.connect(_sender).getPercentages(1);

  console.log("Percentages: WETH: ", percentage1.toString());
  console.log("Percentages: AAVE: ", percentage2.toString());
