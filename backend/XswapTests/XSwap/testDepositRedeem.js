...
async function main() {
  ...
  console.log("Creating ✖ XSwap (Index  Token)...");
  const _Pumpkin = await ethers.getContractFactory("IndexTokenNew");
  const Pumpkin = await _Pumpkin.deploy(
    _sender.address,
    [AAVE.address, WETH.address],
    [50, 50],
    "pumpkins",
    "HWEEN"
  );

  console.log("Xswap address: ", Pumpkin.address);
  ...
  console.log("Owner index Balance: ", pumpkinBalance.toString());
  ...
  console.log("XSWAP-INDEX AAVE Balance: ", AAVEPumpkinBalance.toString());
  console.log("XSWAP-INDEX WETH Balance: ", wethPumpkinBalance.toString());
  ...
  const AAVEOwnerBalance2 = await AAVE.connect(aaveWhale).balanceOf(
    _sender.address
  );
  const wethOwnerBalance2 = await WETH.connect(wethWhale).balanceOf(
    _sender.address
  );
